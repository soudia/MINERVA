...
class MiGRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, amplitudes, scope = None):
        """
           :param inputs: inputs data to pass the GRU cell
           :param state: previous cell state
           :param amplitudes: action (amp_u, amp_c) sampled
               from the policy network to modulate the reset gate
               and the memory cell c
        """
        amp_r, amp_c = amplitudes
        ...
        with tf.variable_scope(scope, reuse = self.reuse):
            ...
            Urh = self.mulWeights(amp_r * r * state, self.numUnits, self.numUnits, name = "Uhl")
            c = self.activation(self.addBiases(Wx, Urh, self.numUnits, name = "2"))

            newH = u * state + (1 - u) * amp_c * c # switch u and 1-u?
        return newH, newH
